...
export default function InquilinoCard({ inq, ipcExtra, onEditar, onEliminar, onPagos, onAplicar, modulo }: Props) {
  ...
  const nPagos = inq.pagos_negro_count || 0
    ...
    <div className="card" style={{ marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          <span className={`badge ${mora > 0 ? 'badge-red' : 'badge-green'}`}>{mora > 0 ? 'En mora' : 'Al día'}</span>
          {(inq.actualizaciones_count || 0) > 0 && <span className="badge badge-blue">{inq.actualizaciones_count} act.</span>}
          {nPagos > 0 && <span className="badge badge-amber">{nPagos} pagos</span>}
        </div>
      </div>
      ...
      {mora > 0 && (
        <div className="alert-mora">
          ⚠️ Mora acumulada: <strong>{fmt(mora)}</strong> ({inq.diasmora} día{inq.diasmora > 1 ? 's' : ''} × {fmt(moraDia)}/día · 10% del total blanco)
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 12 }}>
        <div className="monto-box">
          <div className="lbl" style={{ color: '#16a34a' }}>Socia A (blanco)</div>
          <div className="val" style={{ color: '#16a34a' }}>{fmt(bt / 2)}</div>
        </div>
        <div className="monto-box">
          <div className="lbl" style={{ color: '#16a34a' }}>Socia B (blanco)</div>
          <div className="val" style={{ color: '#16a34a' }}>{fmt(bt / 2)}</div>
        </div>
        {na > 0 ? (
          <div className="monto-box">
            <div className="lbl" style={{ color: '#d97706' }}>c/socia negro</div>
            <div className="val" style={{ color: '#d97706' }}>{fmt(na / 2)}</div>
          </div>
        ) : (
          <div className="monto-box">
